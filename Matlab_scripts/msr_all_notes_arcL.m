...
all_notes_matrix = [note_starts, note_ends, note_durs, INI, pk_amps, pk_times, qpk_amps, qpk_times, note_rmss, rel_pk_amps, rel_qpk_amps, rel_note_rmss, rel_pk_tm, rel_qpk_tm, rel_INI, notes_max_Hz, notes_min_Hz, notes_FM, notes_resid, notes_DF,arcL,chordL]; %Note this assumes FM fit by quadratic, which is default.

labels = char('note.starts', 'note.ends', 'note.durs', 'INI', 'pk.amps', 'pk.times', 'qpk.amps', 'qpk.times', 'note.rmss', 'rel.pk.amps', 'rel.qpk.amps', 'rel.note.rmss', 'rel.pk.tm', 'rel.qpk.tm', 'rel.INI', 'max.Hz', 'min.Hz', 'FMa', 'FMb', 'FMc', 'FM.resid', 'note.DF','arcL','chordL');
